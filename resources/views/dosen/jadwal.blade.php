...
<div x-data="{ search: '' }">
    <div class="mb-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h2 class="text-2xl font-bold">Jadwal Mengajar</h2>
                <p class="text-sm font-medium text-slate-500 dark:text-[#F8FAFC]/70 mt-1">Daftar seluruh jadwal mengajar Anda semester ini.</p>
            </div>
...
@php
    ...
    $jadwalGrouped = $jadwal->sortBy(function($j) use ($hariOrder) {
        return $hariOrder[$j->hari] ?? 99;
    })->groupBy('hari');
@endphp

<div>
    @foreach($jadwalGrouped as $hari => $jadwalHariIni)
    <h3 class="text-xl font-bold text-slate-900 dark:text-[#F8FAFC] mb-4 mt-8">{{ $hari }}</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        @foreach($jadwalHariIni as $j)
        @php
            $sesiAktif = \App\Models\SesiAktif::where('jadwal_id', $j->id)->where('is_aktif', true)->first();
            $today = now()->toDateString();
            $isToday = (\Carbon\Carbon::parse($today)->locale('id')->isoFormat('dddd') == $hari);
        @endphp
        <a href="{{ route('dosen.jadwal.detail', $j->id) }}" x-show="search === '' || '{{ strtolower($j->matakuliah->nama_matkul) }}'.includes(search.toLowerCase()) || '{{ strtolower($j->matakuliah->kode_matkul) }}'.includes(search.toLowerCase())" class="bg-white dark:bg-[#0F172A] rounded-xl border border-gray-200 dark:border-[#F8FAFC] p-6 flex flex-col hover:border-blue-500 dark:hover:border-blue-500 hover:shadow-md transition-all relative overflow-hidden group">
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-[#F8FAFC]/20 flex justify-between items-center">
                ...
                <span class="text-blue-600 dark:text-[#0062FF] text-sm font-medium flex items-center gap-1">
                    Buka Kelas <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </span>
            </div>
        </a>
        @endforeach
