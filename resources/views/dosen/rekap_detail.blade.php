...
<div class="mb-6" x-data="{ search: '' }">
    <div class="flex items-center justify-between mb-4 flex-wrap gap-4">
        <div class="flex items-center gap-4">
            <a href="{{ route('dosen.rekap') }}" class="p-2 bg-white dark:bg-[#0F172A] rounded-xl text-slate-500 dark:text-[#F8FAFC]/70 hover:text-slate-900 dark:hover:text-[#F8FAFC] hover:bg-slate-50 dark:hover:bg-[#1E293B] border border-gray-200 dark:border-[#F8FAFC]/20 transition-colors">
            ...
            <div>
                ...
                <p class="text-slate-500 dark:text-[#F8FAFC]/70 mt-1">Jadwal: {{ ucfirst($jadwal->hari) }}, {{ substr($jadwal->jam_mulai, 0, 5) }} | Ruangan: {{ $jadwal->ruangan->nama_ruangan }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            <a href="{{ route('dosen.rekap.export', $jadwal->id) }}" class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors whitespace-nowrap">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-white/50 dark:bg-[#1E293B]/50 border-b border-gray-200 dark:border-[#F8FAFC]/20 text-slate-900 dark:text-[#F8FAFC] text-sm">
                    ...
                    <th class="p-4 font-semibold text-center text-red-500">Alpa</th>
                    <th class="p-4 font-semibold text-center">Total Ptm</th>
            ...
            <tbody class="text-sm text-slate-600 dark:text-[#F8FAFC]/80 divide-y divide-[#F8FAFC]/10">
                ...
                @php
                    // Alpa dihitung dari total pertemuan berjalan dikurangi kehadiran/izin/sakit
                    $totalKehadiran = $mhs['total_hadir'] + $mhs['total_izin'] + $mhs['total_sakit'];
                ...
                <tr x-show="search === '' || '{{ strtolower($mhs['nama']) }}'.includes(search.toLowerCase()) || '{{ strtolower($mhs['nomor_identitas']) }}'.includes(search.toLowerCase())" class="hover:bg-slate-50 dark:hover:bg-[#1E293B]/50 transition-colors">
                    ...
                    <td class="p-4 font-mono text-xs">{{ $mhs['nomor_identitas'] }}</td>
                    <td class="p-4 font-medium text-slate-900 dark:text-[#F8FAFC]">{{ $mhs['nama'] }}</td>
                    <td class="p-4 text-center font-bold text-green-600 dark:text-green-400">{{ $mhs['total_hadir'] }}</td>
                    <td class="p-4 text-center font-bold text-yellow-600 dark:text-yellow-400">{{ $mhs['total_izin'] }}</td>
                    <td class="p-4 text-center font-bold text-blue-600 dark:text-blue-400">{{ $mhs['total_sakit'] }}</td>
                    <td class="p-4 text-center font-bold text-red-600 dark:text-red-400">{{ $alpa }}</td>
                    <td class="p-4 text-center font-semibold">{{ $totalPertemuan }}</td>
                    <td class="p-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <span class="font-bold {{ $persentase >= 75 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                {{ $persentase }}%
                            </span>
                            <div class="w-16 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                <div class="h-full {{ $persentase >= 75 ? 'bg-green-500' : 'bg-red-500' }}" style="width: {{ $persentase }}%"></div>
                            </div>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="9" class="p-8 text-center text-slate-500 dark:text-[#F8FAFC]/50">
