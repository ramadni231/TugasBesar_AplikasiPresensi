...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-[#F8FAFC]">Halo, Bapak/Ibu {{ Auth::user()->nama }}</h2>
            <p class="text-sm font-medium text-slate-500 dark:text-[#F8FAFC]/70 mt-1">Daftar jadwal mengajar Anda untuk hari ini.</p>
        </div>
...
@if(count($jadwalHariIni) == 0)
    <div class="bg-white dark:bg-[#0F172A] rounded-xl border border-gray-200 dark:border-[#F8FAFC] p-8 text-center">
        ...
        <p class="text-slate-500 dark:text-[#F8FAFC]/70">Anda tidak memiliki jadwal mengajar hari ini.</p>
    </div>
@else
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        @endphp
        <a href="{{ route('dosen.jadwal.detail', $j->id) }}" class="bg-white dark:bg-[#0F172A] rounded-xl border border-gray-200 dark:border-[#F8FAFC] p-6 flex flex-col hover:border-blue-500 dark:hover:border-blue-500 hover:shadow-md transition-all relative overflow-hidden group">
            @if($sesiAktif)
                <div class="absolute top-0 right-0 bg-green-500 text-white px-3 py-1 rounded-bl-xl font-medium text-xs shadow-sm z-10">
                    Sesi Aktif
                </div>
            @endif
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-[#F8FAFC]/20 flex justify-between items-center">
                ...
                <span class="text-blue-600 dark:text-[#0062FF] text-sm font-medium flex items-center gap-1">
                    Lihat Detail <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </span>
            </div>
        </a>
        @endforeach
